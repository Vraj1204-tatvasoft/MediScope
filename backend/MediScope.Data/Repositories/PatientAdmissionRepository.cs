...
namespace MediScope.Data.Repositories
{
    public class PatientAdmissionRepository : IPatientAdmissionRepository
    {
        ...
        public async Task DischargePatientAsync(Guid admissionId, string dischargeNotes)
        {
            var sql = "CALL sp_discharge_patient(@p0, @p1)";
            await _context.Database.ExecuteSqlRawAsync(sql, admissionId, dischargeNotes);
        }
