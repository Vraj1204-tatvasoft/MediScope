    ...
    public class RetryBatchJob
    {
        ...
        public async Task ExecuteAsync(Guid broadcastId, int batchNumber, CancellationToken ct)
        {
            try
            {
                ...
                {
                    _logger.LogInformation("RetryBatchJob: no Failed recipients in batch {Batch} for broadcast {Id}.", batchNumber, broadcastId);
                    return;
                }

                _logger.LogInformation("RetryBatchJob: retrying batch {Batch} — {Count} failed recipients for broadcast {Id}", batchNumber, recipients.Count, broadcastId);
                ...
                if (sent > 0)
                {
                    await _repository.IncrementBroadcastCountsAsync(
                        broadcastId,
                        sentDelta: sent,
                        failedDelta: -sent
                    );
                ...
                _logger.LogInformation(
                    "RetryBatchJob: batch {Batch} retry complete — Sent={Sent} Failed={Failed} for broadcast {Id}",
                    batchNumber, sent, failed, broadcastId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "RetryBatchJob: unhandled exception in batch {Batch} for broadcast {Id}",
        ...
        {
            _logger.LogWarning(
                "RetryBatchJob: SMS not yet supported. Recipient={Id}", recipient.Id);
